...
@Service
@FeignClient(url = "${jsarh.api.url}", fallback = FeriasJSarhClientFallBackImpl.class, name = "ferias")
public interface FeriasJSarhClient {

    @GetMapping(value = "/v1/sarh/servidores/{matricula}/ausencias/ferias", produces = "application/json")
    List<FeriasJSarhResponse> buscaFeriasServidorPorPeriodo(
    ...
    Optional<FeriasJSarhResponse> buscaFeriasServidorNoDia(
            @PathVariable("matricula") String matricula,
            @PathVariable("dia")
